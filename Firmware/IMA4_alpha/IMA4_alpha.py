...
#               2016
# script for the IMA4_alpha in APC emulation mode
...
class IMA4_alpha(ControlSurface):
    __doc__ = " Script for IMA4_alpha version"

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in IMA4_alpha._active_instances:
    ...
    def _do_combine(self):
        if self not in IMA4_alpha._active_instances:
            IMA4_alpha._active_instances.append(self)
            IMA4_alpha._combine_active_instances()


    def _do_uncombine(self):
        if ((self in IMA4_alpha._active_instances) and IMA4_alpha._active_instances.remove(self)):
    ...
    def _setup_session_control(self):
        ...
        self._scene_launch_buttons = [self._note_map[SCENELAUNCH[index]] for index in range(4) ]
        self._track_stop_buttons = [self._note_map[TRACKSTOP[index]] for index in range(4) ]
